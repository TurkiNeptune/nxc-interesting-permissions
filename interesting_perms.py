...
class NXCModule:
    ...
    description = 'Enumerates LDAP for critical permissions and extended rights. Supports token group permissions. -options: builtin=1 to show all builtin attributes, self=1 to see all security attributes of your current login user, tokengroup=1 parses your groups and shows permissions for those groups as well.'
        ...
        try:
            ...
            for entry in conn.entries:
                ...
                for ace in sd['Dacl'].aces:
                    ...
                    if mask & 0x10000000: acls[sid].add("GenericAll")
                    if mask & 0x40000000: acls[sid].add("GenericWrite")
                    if mask & 0x00040000: acls[sid].add("WriteDacl")
                    if mask & 0x00080000: acls[sid].add("WriteOwner")
                ...
                for sid, rights in acls.items():
                    ...
                    if "_gc" in rights: rights.remove("_gc")
                    if "_gca" in rights: rights.remove("_gca")
                    ...
                    if rights:
                        ...
                        if self.tokengroup_mode:
                            ...
                            if current_username not in t_name.lower():
                                continue

                        r_str = ", ".join(sorted(list(rights)))
